...
import {
  ...
  type Feedback, type User, type StudentRegistration,
} from "../api";
...
export function LoginScreen({ onLogin, onRegister }: { onLogin: (email: string, password: string, role: User["role"]) => Promise<void>; onRegister: (input: StudentRegistration) => Promise<void> }) {
  ...
  const choose = (next: User["role"]) => {
    setRole(next); setEmail(next === 'teacher' ? demo.teacher.email : ''); setPassword(next === 'teacher' ? demo.teacher.password : ''); setRegistering(false); setError("");
  };
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setBusy(true); setError("");
    try { if (role === 'student' && registering) await onRegister({ email, password, fullName, studentId }); else await onLogin(email, password, role); }
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_15%_10%,#dbeafe,transparent_35%),#f6f8fb] px-4 py-12 text-slate-900">
      <div className="mx-auto grid min-h-[80vh] max-w-6xl overflow-hidden rounded-[2rem] border border-indigo-100 bg-white shadow-2xl shadow-indigo-900/10 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <section className="flex flex-col justify-center p-7 sm:p-12">
          <p className="text-sm font-bold uppercase tracking-[0.16em] text-indigo-600">{registering ? 'Đăng ký học sinh / sinh viên' : 'Đăng nhập'}</p>
          ...
          <div className="mt-8 grid grid-cols-2 rounded-2xl bg-slate-100 p-1" role="tablist" aria-label="Vai trò đăng nhập">
            {(["teacher", "student"] as const).map((item) => <button key={item} role="tab" disabled={busy} aria-selected={role === item} className={`rounded-xl px-3 py-3 text-sm font-bold transition ${role === item ? "bg-white text-indigo-700 shadow-sm" : "text-slate-600 hover:text-slate-900"}`} onClick={() => choose(item)}>{item === "teacher" ? "Giảng viên" : "Học sinh / sinh viên"}</button>)}
          </div>
          <form onSubmit={submit} className="mt-7 space-y-5">
            {role === 'student' && registering && <>
              <label className="block"><span className="mb-2 block text-sm font-bold">Họ và tên</span><input className={inputClass} required minLength={2} maxLength={100} autoComplete="name" disabled={busy} value={fullName} onChange={e => setFullName(e.target.value)} /></label>
              <label className="block"><span className="mb-2 block text-sm font-bold">Mã số sinh viên / học sinh</span><input className={inputClass} required minLength={5} maxLength={30} pattern="[A-Za-z0-9-]{5,30}" placeholder="Ví dụ: 2A202602785" disabled={busy} value={studentId} onChange={e => setStudentId(e.target.value)} /></label>
            </>}
            ...
            <label className="block"><span className="mb-2 block text-sm font-bold">Mật khẩu</span><input className={inputClass} type="password" autoComplete={registering ? "new-password" : "current-password"} minLength={registering ? 8 : undefined} maxLength={128} disabled={busy} required value={password} onChange={(event) => setPassword(event.target.value)} /></label>
            <p className="rounded-xl bg-indigo-50 p-3 text-xs leading-5 text-indigo-800">{role === 'teacher' ? 'Tài khoản giảng viên đã được điền sẵn.' : registering ? 'Điền đầy đủ họ tên, MSSV, email và mật khẩu ít nhất 8 ký tự để đăng ký.' : 'Đăng nhập bằng tài khoản đã đăng ký để gửi góp ý.'}</p>
            {error && <p role="alert" className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}
            <button disabled={busy} className="w-full rounded-xl bg-slate-950 px-5 py-3.5 font-bold text-white transition hover:bg-indigo-800 disabled:opacity-60">{busy ? 'Đang xử lý...' : registering ? 'Đăng ký và vào trang học sinh' : `Vào trang ${role === "teacher" ? "giảng viên" : "học sinh"}`}</button>
            {role === 'student' && <button type="button" disabled={busy} onClick={() => { setRegistering(!registering); setPassword(''); setError(''); }} className="w-full rounded-xl border border-indigo-200 px-4 py-3 text-sm font-bold text-indigo-700 hover:bg-indigo-50 disabled:opacity-60">{registering ? 'Đã có tài khoản? Đăng nhập' : 'Chưa có tài khoản? Đăng ký'}</button>}
          </form>
        </section>
      </div>
    </main>
