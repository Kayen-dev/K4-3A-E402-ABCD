...
## -1 · Chốt cách đọc khi đánh giá

Pipeline này đúng khi được đọc theo thứ tự ưu tiên sau:

1. **Core bắt buộc là C3**: tìm nguồn, chấm nguồn, tạo hồ sơ nguồn, viết 5 câu có citation theo câu, bỏ nguồn thì chỉ rewrite câu phụ thuộc.
2. **C2 là bước chất lượng nằm sau C3**: soát văn nói/finding giúp tăng điểm chất lượng, nhưng không được làm hỏng hoặc che mất demo C3.
3. Nếu giữ app có cả bước 5 C2, thì `spec.md` phải cập nhật lát cắt thành **C3 + C2**, kèm changelog.
4. Nếu không sửa `spec.md`, khi trình bày phải nói rõ C2 là phần làm vượt, còn lát cắt nộp chính vẫn là C3.

Vì vậy phần thêm C2 là đúng, nhưng để "pipeline đúng" khi chấm thì phải tránh mâu thuẫn giấy tờ: brief/spec/demo phải cùng nói một câu chuyện.

## -0 · Data fixture để implement

Pipeline này dùng data local trong:

```text
D:\K4-3A-E402-ABC\data\studio-pack\c3-scriptscout
```

Chỉ dùng gói `c3-scriptscout` cho prototype chính, vì đây là gói đúng với bài ScriptScout. Các gói `c4-storyboardai` và `c5-feedbackradar` không dùng cho luồng chính.

File cần đọc:

| File | Vai trò trong pipeline |
|---|---|
| `mau-kich-ban.md` | Mẫu xuất `kich-ban.md`. |
| `chu-de-goi-y.md` | Input mẫu để chạy thử. |
| `vi-du/ho-so-nguon-mau.json` | Fixture cho `nguon` và `fact/thongTin`. |
| `vi-du/kich-ban-co-nguon.json` | Fixture cho câu đã nối với nguồn. |
...
nguon[]                   -> Source[]
thongTin[]                -> Fact[]
thongTin[].bangChung[]    -> Evidence[]
cau[]                     -> Sentence[]
cau[].nguon[]             -> Sentence.fact_ids[]
```

URL trong hồ sơ nguồn mẫu là URL giả `.test`, chỉ dùng để mock cấu trúc. Không commit raw data pack vào repo nộp bài; chỉ commit code, docs, fixture nhỏ do đội tự tạo nếu cần.
...
## 10 · Sửa để pipeline khớp bản đánh giá

Chốt hướng: **giữ C2 trong pipeline**, nhưng sửa tài liệu quanh nó để không bị coi là lệch scope.

Việc cần sửa trong `spec.md`:

1. Ở §4, đổi lát cắt từ thuần C3 thành: hệ thống tìm 3 nguồn, chấm nguồn, viết 5 câu có citation, cho loại nguồn để rewrite cục bộ, sau đó có một nút soát văn nói để đánh dấu finding cần người duyệt.
2. Ở phần prototype nhắm tới, thêm bước thật: `soát văn nói bằng nút bấm, hiển thị finding và cho người dùng duyệt`.
3. Ở §7 kiểm thử, thêm metric C2: false positive trên đoạn sạch, số finding đúng, và việc chỉ quét lại câu vừa rewrite.
4. Ở §9 changelog, ghi rõ ngày 17/09/2026: mở rộng lát cắt từ C3 sang C3 + C2 vì bản build có bước soát văn nói.

Việc cần giữ trong demo:

1. Demo C3 trước: nguồn → citation → bỏ nguồn → rewrite cục bộ.
2. Sau đó mới bấm soát văn nói C2.
3. Khi bỏ nguồn và rewrite, chỉ chạy lại C2 trên câu vừa bị rewrite, không quét lại toàn bộ như một hành vi bắt buộc.
4. Luôn nói rõ C2 là lớp kiểm chất lượng sau khi câu đã có nguồn, không phải thay thế citation.

Nếu không kịp sửa `spec.md`, cách trình bày an toàn là: "Pipeline chính theo spec là C3; bước C2 trong bản build là phần mở rộng để kiểm chất lượng văn nói."
